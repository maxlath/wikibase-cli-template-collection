...
const getEntityClaims = async guids => {
  normalizedGuids = guids.map(normalizeGuid)
  const id = getEntityIdFromGuid(guids[0])
  const entity = await getEntity(id)
  const claimsByGuid = {}
  for (const propertyClaims of Object.values(entity.claims)) {
    for (const claim of propertyClaims) {
      const normalizedGuid = normalizeGuid(claim.id)
      if (normalizedGuids.includes(normalizedGuid)) {
        claimsByGuid[claim.id] = claim
      }
    }
  }
  return claimsByGuid
